...
func (u *baseUOW) Close(ctx context.Context) {
	...
	// Dolt and could return a dirty session to database/sql's pool. A later START
	// TRANSACTION commits pending work before opening the next transaction, so
	// cleanup must have its own bounded lifetime.
